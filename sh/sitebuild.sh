...
function __fetchAll() {
    ...
    while read -r line; do
        ...
        appid="$(cut -d^ -f2 <<< "$line")"
        title="$(cut -d^ -f3- <<< "$line")"
        curl --retry 40 --retry-delay 7 --retry-max-time 400 --retry-all-errors -f "https://store.steampowered.com/api/appdetails?appids=$appid" > .workdir/appdetails-curl.txt || return 5
